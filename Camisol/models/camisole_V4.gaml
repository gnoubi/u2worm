...
global
{
	...
	float pop_oligo -> {sum (oligotrophe collect(each.C))};
...
//			write "Cl ("+C_labile+" "+myself.enzyme_Cl/taille+") "+ dec_Cl;
			C_labile  <- C_labile - dec_Cl;
			float dec_Cr <- min([C_recalcitrant,myself.enzyme_Cr/taille]);
//			write "Cr ("+C_recalcitrant+" "+myself.enzyme_Cl/taille+") "+dec_Cr;
...
grid particle width:3 height:3 schedules:[]
...
species microbes_P //skills:[soil_process]
{
	...
	action growth {
		...
		write "pop new ind " + new_individual;
	 ...
	 display "CO2" type: java2D 
		{
			...
			{
				data "CO2_produit" value: world.CO2_produit color: #blue style: spline;
			}
