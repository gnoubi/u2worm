...
global
{
	...
	reflex truc when:every(20#day) and cycle > 10 and false
	{
		int nb_om <- length(organic_particles);
		float OM <-  1.5#gram/(#cm*#cm)*world.shape.area;
		ask organic_particles{
			C_labile <- 0.5* OM/nb_om;
			C_recalcitrant <- 0.5* OM/nb_om;
			N <-0.00132#gram/(#cm*#cm)*world.shape.area;
			P <- 0.00077#gram/(#cm*#cm)*world.shape.area;
		}
	}
 ...
 */			
	...
	reflex displayee
	{
	//	write "data "+ sum(particle collect(each.N))+" "+ sum(particle collect(each.P));
	//	write "res data "+ sum(pores collect(each.dam.dim[0]))+" "+ sum(pores collect(each.dam.dim[1]));
...
{	//solution organique du sol
	...
	action decompe
	{
		...
		ask organics 
		{
			...
			float dec_P <- max([0,min([P,myself.enzyme_P/taille])]);
			P <- P - dec_P;
			add_P <- add_P + dec_P;
			
			float dec_N <- max([0,min([N,myself.enzyme_N/taille])]);
			N <- N - dec_N;
			add_N <- add_N + dec_N;
		}
...
grid particle width:30 height:30 
{
	...
	init
	{
		...
		switch(my_type)
		{
			...
			match ORGANIC { 
					C_labile <- (soil_characteristics at "C"/2)*self.shape.area;
...
species microbes_P 
{
	...
	float P ;
	// Mon estomac
	float assimilated_C <- 0#gram; 
	float assimilated_N <- 0#gram; 
	float assimilated_P <- 0#gram; 
	// ce que je percois
	float income_C <- 0#gram; 
	...
	float decompose
	{
		//write species(self).name + " "+C_wanted+ " "+C;
		ask porous_cell.dam
		{
	...
	{
		CO2_producted <- 0.0;
		sleeping <-C_wanted=0?true: (income_C/C_wanted) >=threshold_DOC;
...
species nematode
{
	...
	float predation_rate <- 0.00001#gram/#day; // 0.5E-9#gram/#mn; //un nematode mange entre 10K et 100k bacterie par jour -> moins de 10k il dort
	...
	action eat 
	{
		...
		ask shuffle(my_pore.populations)
		{
			...
			float N_to_catch <- max([0,min([N,N * C/myself.available_C])]);
			float P_to_catch <- max([0,min([P,P * C/myself.available_C])]);
			...
			myself.my_pore.dam.dom[2] <-myself.my_pore.dam.dom[2] + C_to_catch + assi_C_to_catch;
