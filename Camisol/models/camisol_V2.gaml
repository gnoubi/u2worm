...
species Dom schedules:[] //solution organique du sol
{
	float C;
	float N;
	float P;
...
species Dim schedules:[]//solution inorganique du sol
{
	float N;
	float P;
}
...
{	//					N	P	C
	...
	action decompe(float f_Rr,float f_Cl,float f_P,float f_N, float volume)
	{
		list<particle> ps <- associated_particle.my_neighbors;
		list<particle> organics <- ps where(each.my_type = "organic");
		
		float final_vol <- volume;
		loop while:final_vol >0
		{
			float local_volume <- final_vol / length(organics);
...
species microbes_P skills:[soil_process]
{
	...
	reflex respirate
	{
		float production_co2 <- C * taux_respiration;
		C <- C  - production_co2; 
		CO2_produit <- production_co2 + CO2_produit;
	}
	
	reflex growth { //when: threshold_DOC*population > porous_cell.dam.C {
		population <- population* (1 - (self.porous_cell.total_population) / K)  * (step/dividing_time) + population;
		write population;
	}
}
